 ...
 * Criteria:
 * - val can't be array
 * - parseFloat(val) can't be NaN
 * - parseFloat(val) must be a safe integer
 * - cannot have a leading zero or space
 * @private
 */
function isSafeNumber(val) {
    ...
    return !isArray(val)
        && !isNaN(num)
        && (typeof val !== typeOfString || !leadZeroPatt.test(val))
        && num <= maxSafeInt
        && num >= minSafeInt;
}

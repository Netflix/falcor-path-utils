...
function getRangeSize(range) {
    ...
    if (to != null) {
        if (isNaN(to) || parseInt(to, 10) != to) { // eslint-disable-line eqeqeq
            throw new Error("Invalid range, 'to' is not an integer: " + JSON.stringify(range));
        }
        var from = range.from || 0;
        if (isNaN(from) || parseInt(from, 10) != from) { // eslint-disable-line eqeqeq
            throw new Error("Invalid range, 'from' is not an integer: " + JSON.stringify(range));
        }
        if (from <= to) {
            return (to - from) + 1;
        } else {
            throw new Error("Invalid range, 'to' smaller than 'from': " + JSON.stringify(range));
        }
    } else if (length != null) {
        if (isNaN(length) || parseInt(length, 10) != length) { // eslint-disable-line eqeqeq
